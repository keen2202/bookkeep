...
一款主打「3 秒记一笔」的个人/家庭记账应用：本地优先、乐观写入、离线可用，通过 op-log 增量同步实现多设备/多成员最终一致。覆盖 P0 核心闭环（极速记账、多账户、分类、预算、报表、云同步、隐私锁）与 P1 差异化功能（多账本共享、自动记账、备份导出、周期分期、多币种、报表内日历视图）。
...
| 自动/智能记账 | 支付宝/微信 CSV 解析导入（去重 + 幂等）、Android 通知/短信解析；全部经确认页才入账，杜绝静默写库 |
| 备份 + 导出 | CSV 导出（区间/账本/类型筛选）、AES-256-GCM 加密备份（PBKDF2 派生密钥，口令不落盘）、WebDAV 上传/恢复（仅 HTTPS）、单事务原子恢复 |
| 周期/分期记账 | 设置页独立入口（规则创建/编辑/删除）、日/周/月/季/年频率 + 按频率的锚点选项（月初/月中/月末等）、RRULE 子集展开引擎（月末回退、闰年、跨年）、启动补跑幂等、分期计划（等额末笔补差，合计误差 0） |
| 多币种 + 汇率 | 156 种 ISO 4217 币种 seed、流水三要素（原币/币种/汇率快照）、24h TTL 汇率缓存 + 手动汇率、报表按主币种折算（定点整数 half-up，精度到分） |
| 日历/现金流视图（并入报表） | 报表页「图表/日历」双视图：月历日格收支净额（支出红/收入绿）、点击某天查看当天账单明细、30 天滑动窗口现金流趋势图、月份懒加载 |
