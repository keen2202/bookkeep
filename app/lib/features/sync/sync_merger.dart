...
class SyncMerger {
    ...
    await db.transaction(() async {
      for (final r in [...creates, ...others]) {
        try {
          ...
          // 单 op 失败（畸形 payload/约束）不中断整批合并（评审 H1）；记日志便于排查
          debugPrint('[sync_merger] apply failed ${r.entity}/${r.entityId}: $e');
        }
      }
  ...
  Future<bool> _applyUpdate(String entity, String remoteId, Map<String, dynamic>? payload) async {
    // Spec R-18：book 主键即 remote_id（text），无 integer local id
    if (entity == 'book') {
      if (payload == null) return false;
      return _updateBook(remoteId, payload);
    }
  ...
  Future<bool> _updateBook(String remoteId, Map<String, dynamic> p) async {
    final (name, hasName) = _strField(p, 'name');
    final (type, hasType) = _strField(p, 'type');
    if (!hasName && !hasType) return false;
    final updated = await (db.update(db.books)..where((t) => t.id.equals(remoteId))).write(
      BooksCompanion(
        name: hasName ? Value(name ?? '') : const Value.absent(),
        type: hasType ? Value(type ?? 'default') : const Value.absent(),
      ),
    );
    return updated > 0;
  }
  ...
  Future<int?> _createTransaction(String remoteId, Map<String, dynamic> p) async {
    ...
    return db.into(db.transactions).insert(TransactionsCompanion.insert(
          ...
          amountMinor: amount,
          currency: _str(p, 'currency') ?? 'CNY',
          rateSnapshot: Value(_int(p, 'rate_snapshot') ?? kRateScale),
  ...
  Future<bool> _updateTransaction(int localId, String remoteId, Map<String, dynamic> p) async {
    ...
    final categoryRef = _str(p, 'category_id');
    final categoryId = categoryRef == null ? null : await _localIdByRemoteId('category', categoryRef);
    // Spec R-17：分类引用存在但未同步到本地时跳过更新，避免错误置 null
    if (categoryRef != null && categoryId == null) return false;
    final type = _enumFromName(p['type'], TransactionType.values);

    final (amount, hasAmount) = _intField(p, 'amount_minor');
    final (currency, hasCurrency) = _strField(p, 'currency');
    final (rateSnapshot, hasRateSnapshot) = _intField(p, 'rate_snapshot');
    ...
    await (db.update(db.transactions)..where((t) => t.id.equals(localId))).write(
      ...
      currency: hasCurrency ? Value(currency ?? 'CNY') : const Value.absent(),
      rateSnapshot: hasRateSnapshot ? Value(rateSnapshot ?? kRateScale) : const Value.absent(),
